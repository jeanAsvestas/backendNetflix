...
// Select and see the categorazation of movies
...
// Update the categorazation of a movie
    ...
    MovieCategory.findOne({
        ...
        if (moviecategory.id == req.body.id) {
            ...
            moviecategory.save().then(moviecategory => {
                res.status(200).send({ message: `The movie categorazation with id: ${moviecategory.id} was updated successfully` });
                return;
            }).catch(err => {
                res.status(500).send({ message: err.message });
                return;
            ...
            res.status(200).send({ message: `The movie categorazation with id: ${moviecategory.id} has changed` });
        }
    }).catch(err => {
        res.status(500).send({ message: err.message });
    });
...
// Delete the categorazation of a movie
exports.deleteMovieCategory = (req, res) => {
    MovieCategory.destroy({
        ...
        if(deleted == 1) {
            res.status(200).send(`The categorazation of the movie was deleted succesfully`);
        }
        else {
            res.status(500).send(`There is no categorazation of movie with id: ${req.body.id}`);
        }
